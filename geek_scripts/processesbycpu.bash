...
#top -ocpu -F -R -l2 -n20 | grep 'CPU usage:' | tail -1
...
if [ 1 ] ; then
	...
	top -ocpu -F -R -l2 -n20 | tail -21 | awk '{printf "%-8s %-15s %8s %8s %10s\n", $1, $2, $3, $16, $26}' | head -$mycount
else
	# this is from the original website cited at the top of the script
	top -ocpu -F -R -l2 -n20 | grep '^....[1234567890] ' | grep -v ' 0.0% ..:' | cut -c 1-24,33-42,64-77 | head -$mycount
fi
echo
