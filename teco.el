;;; Teco interpreter for Gnu Emacs, version 3.

;; LCD Archive Entry:
;; teco|Dale R. Worley|drw@math.mit.edu
;; |Teco interpreter
;; |91-08-07|version 3|~/packages/teco.el.Z
...
;; Make flow-control commands clear the @-flag, so the @-flag can be statically
;; predicted during skipping of code.
...
;; Philosophy (stolen from somebody):
...
(defvar teco:ctrl-s 0
  "The negative of the last string inserted or searched for.
Set by the S, I, TAB, G, FR, and \\ commands.")
...
(defun teco:set-var (var)
  (if teco:exp-flag1
	...
	(if teco:exp-flag2
	    ...
	    (set var (logior (logand (symbol-value var)
				     (lognot teco:exp-val2))
			     teco:exp-val1))
  ...
  (if teco:exp-flag1
	...
	(set variable
		 ...
		 (logior (logand (lognot (symbol-value variable))
				 teco:exp-val2) teco:exp-val1)
	       ;; One argument is the new value
	       teco:exp-val1))
	(setq teco:exp-flag1 nil
	      teco:exp-flag2 nil))
